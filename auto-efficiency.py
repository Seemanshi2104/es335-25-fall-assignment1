...
print("Part (a): Custom Decision Tree (Regression)")
...
print("Part (b): Sklearn Decision Tree Regressor")
...
#This code is written after finding the optimal depth under 10 of Sklearn tree and our custom tree

print("Comparing the predictions at the optimal depth of both the trees")
custom_tree = DecisionTree(criterion="mse", max_depth=6)
custom_tree.fit(X_train, y_train)
y_hat_custom = custom_tree.predict(X_test)

sk_tree_depth5 = DecisionTreeRegressor(criterion="squared_error", max_depth=5, random_state=42)
sk_tree_depth5.fit(X_train, y_train)
y_hat_sk5 = sk_tree_depth5.predict(X_test)

plt.figure(figsize=(8, 5))
plt.scatter(y_test, y_hat_custom, label="Custom Tree (depth=6)", alpha=0.7, marker="o")
plt.scatter(y_test, y_hat_sk5, label="Sklearn Tree (depth=5)", alpha=0.7, marker="x")
plt.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], "k--")
plt.xlabel("True MPG")
plt.ylabel("Predicted MPG")
plt.title("Custom Tree (depth=6) vs Sklearn Tree (depth=5)")
plt.legend()
plt.grid(True)
plt.show()